...
public class KML_Logger 
{
	...
	public KML_Logger(MyGameGUI myGameGUI) 
	{
		myg=myGameGUI;
	}


	public void KmlObjr() throws ParseException, InterruptedException 
	{
        ...
        int i = 0 ;
        if (myg.getGameService()!= null)
        {
            
                Thread.sleep(100);
        
        ArrayList<Robot> robot = myg.getRobotsList();
        
        ArrayList<Fruit> fru = myg.getFruitsList();
        ...
        for (Robot p : robot) 
        {
            ...
            plmark.withDescription("Mac: " + "\nType: Robot").withOpen(Boolean.TRUE).createAndSetPoint().addToCoordinates(p.getPos().x(), p.getPos().y());
            i++;
